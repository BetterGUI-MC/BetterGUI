  ...
  public StringReplacer getStringReplacer() {
    return new ElementLookupStringReplacer<MenuButton>() {
      ...
      public List<MenuButton> getElements() {
        if (WrappedListButton.this.button != null) {
          return ((ListButton) WrappedListButton.this.button).getButtons().stream().filter(MenuButton.class::isInstance).map(MenuButton.class::cast).collect(Collectors.toList());
        } else {
