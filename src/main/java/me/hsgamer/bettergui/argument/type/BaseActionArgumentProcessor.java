...
public abstract class BaseActionArgumentProcessor implements ArgumentProcessor {
  ...
  public BaseActionArgumentProcessor(ArgumentProcessorBuilder.Input input) {
    ...
    this.onRequiredActionApplier = new ActionApplier(input.parent, MapUtils.getIfFoundOrDefault(options, Collections.emptyList(), "required-command", "required-action", "action", "command"));
    this.onInvalidActionApplier = new ActionApplier(input.parent, MapUtils.getIfFoundOrDefault(options, Collections.emptyList(), "invalid-command", "invalid-action"));
  }
