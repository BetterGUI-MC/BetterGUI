  ...
  public void setFromFile(FileConfiguration file) {
    ...
    for (String key : file.getKeys(false)) {
      if (key.equalsIgnoreCase("menu-settings")) {
        Map<String, Object> settings = new CaseInsensitiveStringMap<>(file.getConfigurationSection(key).getValues(false));
        ...
        if (settings.containsKey(Settings.ARGS)) {
          List<String> args = CommonUtils.createStringListFromObject(settings.get(Settings.ARGS), true);
